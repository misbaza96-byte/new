      ...
      const mainInterval = setInterval(function () {
        ...
        if (a) {
          ...
          if (inGame && config.type === 'follow') {
            ...
            // 🚀 BIẾN NHẬN DIỆN KAMIKAZE (Khi đè Chuột Phải hoặc Shift)
            let isKamikaze = target.rMouseDown || target.shift;
            ...
            if (target.manualMode) {
              ...
              // 💥 LÕI KAMIKAZE: Ép Bot lao thẳng vào điểm chuột, bỏ qua mọi thứ
              if (isKamikaze || target.followMouse) {
            ...
            if (valid) {
              ...
              let angle;
              if (isKamikaze) {
                // Đang Kamikaze: Khóa nòng súng thẳng vào mục tiêu
            ...
            // 💥 XỬ LÝ CHUỘT KAMIKAZE:
            if (isKamikaze) {
              controller.mouseDown(); // Tự động xả đạn liên tục để dọn đường
              controller.mouseUp(2);  // Ép nhả chuột phải (Để Tank Drone như Overseer thả Drone cắn địch thay vì đẩy Drone ra xa)
            } else {
